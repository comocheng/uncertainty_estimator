...
def simulation_wrapper(params):
    ...
    dist_array, gas_out, surf_out, gas_rates, surf_rates = adaptive_simulation.run_simulation(
        mech_yaml,
    ...
    for i in range(len(sample_distances)):
        interpolation_indices.append(int(np.argmin(np.abs(dist_array - sample_distances[i]))))
        # assert np.abs(dist_array[interpolation_indices[i]] - sample_distances[i]) <= 1e-4, 'CSTR size is too large for interpolation'
    ...
    # do linear interpolation to get sample at exact requested distance
    results = []
    for i in range(len(interpolation_indices)):
        idx = interpolation_indices[i]
        distance_results = []  # results at a given distance in the reactor
        for j in output_gas_species_indices:
            slope = (gas_out[idx, j] - gas_out[idx - 1, j]) / (dist_array[idx] - dist_array[idx - 1])
            interpolated_result = gas_out[idx, j] + slope * (sample_distances[i] - dist_array[idx])
            distance_results.append(interpolated_result)
        results.append(distance_results)
