...
def run_simulation(
    ...
    for b in range(n_iter):
        ...
        this_results.append(r.thermo.Y.copy())  # store the initial conditions as the results for the first reactor
    
        print(f"Starting iteration {b+1}/{n_iter} of the backmixing simulation with backmixing fraction {backmixing}")
        for n in range(N_REACTORS):
            signal.alarm(TIMEOUT_SECONDS)  # Set the alarm for the timeout

            # Set the state of the reservoir to match that of the previous reactor
            if use_temperature_profile:
                if prev_results and n < N_REACTORS - 1:  # only apply backmixing if we have previous results and we're not at the last reactor
                    Y_mix = (1 - backmixing) * r.thermo.Y + backmixing * prev_results[n + 1]
                    gas.TDY = temperature_profile[n], r.thermo.DP[0], Y_mix
                else:
            ...
            this_results.append(r.thermo.Y.copy())
        prev_results = this_results
        this_results = []
...
def plot_top_n(result, phase, n=10, outfile=None, species=True, title=None, xlim=None, ylim=None, show=False):
