...
rh_data = 'rh_profiles_smooth.csv'
...
# # THIS WAY CAUSES THE BUG AND DOES NOT REACT
...
def monolithFull(gas, surf, T):
    ...
    for n in range(NReactors):
        ...
        sim.advance_to_steady_state(max_steps=100000, atol=sim.atol/100.0)
...
if show_experiment:
    plt.plot(df['Distance (mm)'].values, df['O2 (mol/min)'].values, label='O2 (exp)', color=colors[0], linewidth=linewidth1)
    plt.plot(df['Distance (mm)'].values, df['CH4 (mol/min)'].values, label='CH4 (exp)', color=colors[1], linewidth=linewidth1)
    plt.plot(df['Distance (mm)'].values, df['H2 (mol/min)'].values, label='H2 (exp)', color=colors[2], linewidth=linewidth1)
    plt.plot(df['Distance (mm)'].values, df['CO (mol/min)'].values, label='CO (exp)', color=colors[3], linewidth=linewidth1)
    plt.plot(df['Distance (mm)'].values, df['CO2 (mol/min)'].values, label='CO2 (exp)', color=colors[4], linewidth=linewidth1)
    
plt.plot(dist_array, gas_out[:, i_O2], label='O2 (sim)', color=colors[0], linestyle='--', linewidth=linewidth2)
plt.plot(dist_array, gas_out[:, i_CH4], label='CH4 (sim)', color=colors[1], linestyle='--', linewidth=linewidth2)
plt.plot(dist_array, gas_out[:, i_H2], label='H2 (sim)', color=colors[2], linestyle='--', linewidth=linewidth2)
plt.plot(dist_array, gas_out[:, i_CO], label='CO (sim)', color=colors[3], linestyle='--', linewidth=linewidth2)
plt.plot(dist_array, gas_out[:, i_CO2], label='CO2 (sim)', color=colors[4], linestyle='--', linewidth=linewidth2)
plt.plot(dist_array, gas_out[:, i_C2H4], label='C2H4 (sim)', color=colors[5], linestyle='--', linewidth=linewidth2)
