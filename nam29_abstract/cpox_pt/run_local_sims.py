...
perturbed_gas = ct.Solution(perturbed_yaml_path, 'gas')
perturbed_surf = ct.Interface(perturbed_yaml_path, 'surface1', [perturbed_gas])
...
for i in range(len(perturbed_gas.species())):

    print(f'perturbing {i} {perturbed_gas.species()[i]}')

    spec_delay_file_gas = os.path.join(results_dir, f'gas_spec_{i:04}_gas_out.npy')
    spec_delay_file_surf = os.path.join(results_dir, f'gas_spec_{i:04}_surf_out.npy')
...
for i in range(len(perturbed_surf.species())):

    print(f'perturbing {i} {perturbed_surf.species()[i]}')

    spec_delay_file_gas = os.path.join(results_dir, f'surf_spec_{i:04}_gas_out.npy')
    spec_delay_file_surf = os.path.join(results_dir, f'surf_spec_{i:04}_surf_out.npy')
    if os.path.exists(spec_delay_file_gas) and os.path.exists(spec_delay_file_surf):
        print(f'skipping {i} because file already exists!')
        continue
    ...
    # run the simulations at condition #j
    base_surf.modify_species(i, perturbed_surf.species()[i])
...
for i in range(len(perturbed_gas.reactions())):

    print(f'perturbing {i} {perturbed_gas.reactions()[i]}')

    rxn_delay_file_gas = os.path.join(results_dir, f'gas_rxn_{i:04}_gas_out.npy')
    rxn_delay_file_surf = os.path.join(results_dir, f'gas_rxn_{i:04}_surf_out.npy')
    if os.path.exists(rxn_delay_file_gas) and os.path.exists(rxn_delay_file_surf):
        print(f'skipping {i} because file already exists!')
        continue
    ...
    # run the simulations at condition #j
    base_gas.modify_reaction(i, perturbed_gas.reactions()[i])

    a = run_simulation(base_gas, base_surf)
    gas_out, surf_out, gas_names, surf_names, dist_array, T_array = a

    np.save(rxn_delay_file_gas, gas_out)
    np.save(rxn_delay_file_surf, surf_out)


for i in range(len(perturbed_surf.reactions())):

    print(f'perturbing {i} {perturbed_surf.reactions()[i]}')

    rxn_delay_file_gas = os.path.join(results_dir, f'surf_rxn_{i:04}_gas_out.npy')
    rxn_delay_file_surf = os.path.join(results_dir, f'surf_rxn_{i:04}_surf_out.npy')
    if os.path.exists(rxn_delay_file_gas) and os.path.exists(rxn_delay_file_surf):
        print(f'skipping {i} because file already exists!')
        continue
    ...
    # run the simulations at condition #j
    base_surf.modify_reaction(i, perturbed_surf.reactions()[i])
