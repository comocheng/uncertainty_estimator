...
species_list, reaction_list = rmgpy.chemkin.load_chemkin_file(gas_chemkin, sp_dict, surface_path=gas_surface)
# -

len(reaction_list)

# +
surface_families = [family for family in database.kinetics.families.keys()]
# for family in database.kinetics.families.keys():
#     print(family)
    
#     train = database.kinetics.families[family].get_training_depository()
#     print(len(train.entries))
#     print()
...
for z in range(len(reaction_list)):
    ...
    if 'surface' not in my_rxn.family.lower() or 'deutschmann2006' in my_rxn.family.lower():
        continue
    other_reactions.append(my_rxn)
    print(z)
    break
#     print(my_rxn.family)
    ...
    for family in surface_families:
        try:
            ...
            kinetics = database.kinetics.families[family].get_kinetics_for_template(template, degeneracy=my_rxn.degeneracy)
#             other_reactions.append(template)
        except (rmgpy.exceptions.ActionError, ValueError, IndexError):
            continue
...
reaction_list[65].reactants[0].molecule[0].get_all_labeled_atoms()

type(reaction_list[65])

database.kinetics.families[family].add_atom_labels_for_reaction(reaction_list[65])





for family in ['Surface_Adsorption_Single']:
#     print(family)
    
    database.kinetics.families[family].add_atom_labels_for_reaction(reaction_list[65])
    template_labels = database.kinetics.families[family].get_reaction_template_labels(reaction_list[65])
    template = database.kinetics.families[family].retrieve_template(template_labels)
    kinetics = database.kinetics.families[family].get_kinetics_for_template(template, degeneracy=reaction_list[65].degeneracy)
...
# +
# how many reactions are we talking?
# -
